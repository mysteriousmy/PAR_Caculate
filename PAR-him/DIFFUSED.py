...
def diffuse(newlist, halfball, v2, r_grid):
    minp = min(halfball[:, 0])
    mint = min(halfball[:, 1])
    minr = min(halfball[:, 2])
    ...
    for i in range(0, len(newlist)):
        ...
        condition_data_index = np.where((halfball[:, 1] <= tdata) & (halfball[:, 1] >= tdata_e) & (
            halfball[:, 0] <= pdata) & (halfball[:, 0] >= pdata_e) & (halfball[:, 2] <= rdata) & (halfball[:, 2] >= rdata_e))[0]
...
def doballtest(name, halfball, r_grid, v2, result_dict, result_lock):
    enewlist = []
    Ngap_num = 0
    theta_grid = int(90 / v2)
    phi_grid = int(360 / v2)
    newlist = [[]
               for row in range(theta_grid * phi_grid * r_grid)]
    enewlist.append(diffuse(newlist, halfball, v2, r_grid))
    for i in enewlist:
        Ngap_num = Ngap_num + i.count([])
    Ngap_num = Ngap_num / ((theta_grid * phi_grid *
                            r_grid) * len(enewlist))

    with result_lock:
        result_dict[name] = Ngap_num
    return round(Ngap_num*100, 2)


# def doball(halfball, r_grid, v2, pool):
#     param_dict = []
#     Ngap_num = 0
#     theta_grid = int(90 / v2)
#     phi_grid = int(360 / v2)
...
def start_cacu_byops(managed_dict, r_grid, v2):
    Ngap_num = 0
    param_dict = []
    num_cores = int(mp.cpu_count())
    pool = mp.Pool(num_cores)
    for namex, datas in managed_dict.items():
        param_dict.append(['task{}'.format(namex), datas, r_grid, v2])
    print(len(managed_dict))
    manager = mp.Manager()
    managed_locker = manager.Lock()
    managed_dict = manager.dict()
    results2 = [pool.apply_async(doballtest, args=(
        name, halfball, r_grid, v2, managed_dict, managed_locker)) for name, halfball, r_grid, v2 in param_dict]
    results2 = [p.get() for p in results2]
    for namex, datas in managed_dict.items():
        Ngap_num = Ngap_num + datas
    return (Ngap_num / len(managed_dict)) * 100
...
if __name__ == "__main__":
   ...
   # theta_grid = int(90/v2)
    ...
    Ngap_num = start_cacu_byops(one_dict, r_grid, v2)
    print(Ngap_num)
