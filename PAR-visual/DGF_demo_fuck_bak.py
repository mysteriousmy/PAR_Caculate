...
def caculate_mp(data, realdata, v0, v1):
...
def opp(name, ops, op, z_v0, v1, realdata, result_dict, result_lock):
    ...
    Ngap = caculate_mp(newdata, realdata, z_v0, v1)
    with result_lock:
...
def create_square(ops, z_v0, v1, realdata):
    param_dict = []
    num_cores = int(mp.cpu_count())
    pool = mp.Pool(num_cores)
    for i in range(0, len(ops)):
        param_dict.append(
            ['task{}'.format(i), ops, ops[i], z_v0, v1, realdata])
    manager = mp.Manager()
    managed_locker = manager.Lock()
    managed_dict = manager.dict()
    results = [pool.apply_async(opp, args=(
        name, ops, op, z_v0, v1, realdata, managed_dict, managed_locker)) for name, ops, op, z_v0, v1, realdata in param_dict]
...
def init_cacu_data(zhigh_percent, realdata, z_v0, v1):
    ...
    result = create_square(filter_xyz, z_v0, v1, realdata)
    return result
